...
#define ALPHA 0.9
#define BETA 5
#define EVAPORATION_CONSTANT 0.95
#define DELTA 100
...
public:
    ...
    void antColonyOptimazation(){
        ...
        std::ifstream infile("bestRes.txt");
        double fileShortestLen;
        if (!infile){
             std::cout << "error opeing inFile" << std::endl;
             fileShortestLen = DBL_MAX;
        }
        else {
            std::string s;
            std::getline(infile, s);
            fileShortestLen = std::stod(s);
        }
        if (currBestDistance < fileShortestLen){
            std::ofstream outfile("bestRes.txt");
             if (!outfile){
                 std::cout << "error opeing outFile" << std::endl;
            }
            outfile << currBestDistance << std::endl;
            for (int i : currBestPath) outfile << i << " ";
        }
